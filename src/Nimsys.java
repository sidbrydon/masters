...
//    private static NimPlayer[] players = new NimPlayer[MAX_NO_OF_PLAYERS];
    ...
    private static void runningCommands(String commandtoRun){
        ...
        String[] arrayCommand = commandtoRun.split(" ", 2);
        ...
        try{
            switch (arrayCommand[0]) 
            {
                ...
                case "exit":
                 ...
                 if (arrayCommand.length != 1) throw new inputException(2,2); 
                 exitFinal();
                 System.out.println();
                 System.exit(0);
                 keepPlaying = false;
                ...
                case "addplayer":
                 if (arrayCommand.length == 1) throw new inputException(2,2);
                 String[] addpArray = arrayCommand[1].split(",", 3);
                 if (addpArray.length < 3) throw new inputException(2,2);
                 else addplayer(addpArray[0],addpArray[1],addpArray[2],false);               
                break;
                
                /* This command will add an automnomous player */
                case "addaiplayer":
                 if (arrayCommand.length == 1) throw new inputException(2,2);
                 String[] addaipArray = arrayCommand[1].split(",", 3);
                 if (addaipArray.length < 3) throw new inputException(2,2);
                 else addaiplayer(addaipArray[0],addaipArray[1],addaipArray[2]);
                break;
                ...
                case "removeplayer":
                 if (arrayCommand.length == 1)
                 {
                   ...
                   if ("y".equalsIgnoreCase(confdeny)) removePlayer();
                   else break;                        
                 }
                 else removePlayer(arrayCommand[1]);
                break;
                    
                /* This case will edit a player */
                case "editplayer":
                 String[] editpArray = arrayCommand[1].split(",", 3);
                ...
                case "resetstats": 
                 if (arrayCommand.length == 1)
                 {
                    ...
                    String confdeny = KEYBOARD.nextLine();
                    confdeny = confdeny.trim();
                    if ("y".equalsIgnoreCase(confdeny)) resetStats();
                    else break;                        
                 }
                 else resetStats(arrayCommand[1]);
                break;

                /* This case will display a player or all players ordered user*/
                case "displayplayer":
                 if (arrayCommand.length == 1) displayPlayer();
                 else displayPlayer (arrayCommand[1]);
                break;
                ...
                case "startgame":
                 ...
                 String[] startgArray = arrayCommand[1].split(",", 4);
                 if (startgArray.length < 4) throw new inputException(2,2);                    
                 int tempinit = Integer.parseInt(startgArray[0]);
                 int tempupbd = Integer.parseInt(startgArray[1]);
                ...
                case "startadvancedgame":
                 ...
                 if (arrayCommand.length == 1) throw new inputException(2,2);
                 arrayCommand[1] = arrayCommand[1].trim();
                 String[] startadvArray = arrayCommand[1].split(",", 3);
                 if (startadvArray.length < 3) throw new inputException(2,2);                    
                 int tempadvi = Integer.parseInt(startadvArray[0]);
                ...
                default:
                 boolean helpCommand = checkCommand (arrayCommand[0]);
    ...
    private static boolean checkCommand (String inputCommand){
        ...
        for (String realcommand : listofcommands) 
        {
            if (realcommand.equals(inputCommand)) rightCommand = true;
        }     
        return rightCommand; 
    } 
    
    /* Tests to see if the file already exists */
    private static boolean fileTest(File forTest){
    ...
    private static void fileImport(File forImport)
    {
        ...
        if (fileTest(Nimsys.playerFile))
        {
            ...
            {    
                ...
                {     
                    ...
                    if(!isaiornot)Nimsys.playersAL.add(
                            readin, new NimHumanPlayer(iuname,isname,ifname) {});
                    else Nimsys.playersAL.add(
                            readin, new NimAIPlayer(iuname,isname,ifname));
    ...
    private static void fileExport ()
    {
        ...
        for (int ops=0; ops < Nimsys.playercount; ops++)
        {
            outputStream.println(
                Nimsys.playersAL.get(ops).getUsername() + "," +
                Nimsys.playersAL.get(ops).getSurname() + "," +
                Nimsys.playersAL.get(ops).getForename() + "," +
                Nimsys.playersAL.get(ops).getPlayed () + "," +
                Nimsys.playersAL.get(ops).getWon() + "," +
                Nimsys.playersAL.get(ops).getPercentwins() + "," + 
...
//        System.out.println("End of program.");
    ...
    {
        if (!(playersAL == null))fileExport();
        else System.exit(0);
    }
    
    /* Method to add the player to the NimPlayer array */
    private static void addplayer(String uname, String sname, 
            String fname, boolean isAI)
    {
        /** This will add and create a NimPlayer to the array of players 
         * Checks to see if the player exists and errors if it does
         */
        int plexists = 0;
        ...
        else
        {
            ...
            {
                String tempname = playersAL.get(addid).getUsername();
            ...
            else System.out.println("The player already exists.");
        }
    /* End of addPlayer */    
    }
    
    /* Method to add and AI player to the NimPlayer array */
    private static void addaiplayer(String uname, String sname, String fname)
    {
    ...
    private static void removePlayer(String userName)
    {
        ...
        if (removepl == false) System.out.println("The player does not exist.");
    /* End of removePlayer */
    }

    /* Find the user with user name, edit there details, but not username */
    private static void editplayer(String editUname, 
            String editSname, String editFname)
    {
        boolean editfind = false;
        for (NimPlayer editPly: playersAL)
        {
        ...
        if (editfind == false) System.out.println("The player does not exist.");
    /*End of editPlayer */
    }
    ...
    {   
        for (NimPlayer resetPly: playersAL) resetPly.setStats();  
    }
    
    /* Resets a specific stats to 0, found by user name */
    private static void resetStats(String resetUname)
    {
        boolean checkreset = false;
        for (NimPlayer resetPly: playersAL)
        {
     ...
     * Display a specific player, found by user name with:
     * Username, Forename, Surname, Games Played and Games Won
        */
    private static void displayPlayer(String dispUser)
    {
        boolean founduser = false;
        for (NimPlayer playersAL1 : playersAL) 
        {
            if (dispUser.equalsIgnoreCase(playersAL1.getUsername())) 
            {
                System.out.println(playersAL1.getUsername() + "," + 
                        playersAL1.getForename() + "," + 
                        playersAL1.getSurname() + "," + 
                        playersAL1.getPlayed() + " games," + 
                        playersAL1.getWon() + " wins");
                founduser = true;
            }
        }    
        if (founduser == false)System.out.println("The player does not exist.");       
    }
    ...
    public static Comparator<NimPlayer> PlrNmComp=new Comparator<NimPlayer>() 
    {
        ...
        @Override
	public int compare(NimPlayer plyr1, NimPlayer plyr2) 
        {
	   String PlayerName1 = plyr1.getUsername().toLowerCase();
	   String PlayerName2 = plyr2.getUsername().toLowerCase();
	...
	public int compare(NimPlayer plyr1, NimPlayer plyr2) 
        {
	   int PlayerWin1 = plyr1.getPercentwins();
	   int PlayerWin2 = plyr2.getPercentwins();
    ...
    private static void sortPlayersbyUser(ArrayList<NimPlayer> copyPlayers)
    {
      
        Collections.sort(copyPlayers, PlrNmComp);
        
        for (NimPlayer copyPlayer : copyPlayers) 
        {
            if (!copyPlayers.isEmpty()) 
            {
                System.out.println
                (
                        copyPlayer.getUsername() + "," + 
                        copyPlayer.getForename() + "," + 
                        copyPlayer.getSurname() + "," + 
                        copyPlayer.getPlayed() + " games," + 
                        copyPlayer.getWon() + " wins"
                );
            } 
            else 
            {
    ...
    private static void sortForRankings(ArrayList<NimPlayer> rankPlayers)
    {
        ...
        for (NimPlayer element: rankPlayers)
        {
            if (index <= 10)
            {    
    ...
    {
        ArrayList<NimPlayer> dispPlayers = new ArrayList<>(MAX_NO_OF_PLAYERS);
    ...
    {
        ArrayList<NimPlayer> statsPlayers = new ArrayList<>(MAX_NO_OF_PLAYERS);
        statsPlayers.addAll(playersAL);
        sortForRankings(statsPlayers);
    }
    
    /* Method called to start playing the game */
    private static void startGame(int is, int ub, String uname1, String uname2)
    {
        /* Play out the game until the end */
        /* Create each player */
        String playerOne = uname1, playerTwo = uname2;
        int upBo = ub, iniNo = is;
        ...
        {   
            ...
            NimGame game1 = new NimGame(
                    iniNo,upBo,
                    uname1,
                    uname2);
            
            /* Return the winner of the game */        
            String winner = game1.getWinner();
               
            /* Add one to the winners stats */
            for (NimPlayer win: playersAL)
            {
    ...
    private static void startadvancedgame (int isadv, 
            String uname1, String uname2)
    {
        /* Play out the game until the end */
        /* Create each player */
        String playerOne = uname1, playerTwo = uname2;
        int upperBound = 2, initialNumber = isadv;
        int firstplpos = 0, index1 = 0, secondplpos = 0, index2 = 0; 
        boolean onecheck = false,twocheck = false, isadvgame = true;
        
        /* Find the first user and confirm they exist */
        for (NimPlayer one: playersAL)
        {
        ...
        {   
            /* Create the game object then start the game */
            NimGame advGame1 = new NimGame(
                    initialNumber,
                    upperBound,
                    playersAL.get(firstplpos).getUsername(),
                    playersAL.get(secondplpos).getUsername(),
                    isadvgame
                    );
        
            String winner = advGame1.getWinner();
